...
def run_wizard(
    path: Path, *, input_fn: Callable[[str], str] = input, output: Callable[[str], None] = print
    ...
    mode = _ask("Adoption mode (auto, adopt, or greenfield)", "auto", input_fn).lower()
    while mode not in {"auto", "adopt", "greenfield"}:
        output("Choose auto, adopt, or greenfield.")
        mode = _ask("Adoption mode", "auto", input_fn).lower()
    ...
    browsers = (
        install
        and bool(detection.html or detection.css or detection.frameworks)
        and _yes("Install Playwright Chromium for browser checks now", False, input_fn)
    )
    ...
    if detection.html or detection.css or detection.frameworks:
        inferred_url = (
            "http://127.0.0.1:5173" if "vite" in detection.frameworks else "http://127.0.0.1:3000"
        )
        base_url = _ask(
            "Local web URL used by browser and performance checks", inferred_url, input_fn
        )
    ...
    output(
        f"Doctor: {doctor['status']} · {doctor['counts']['error']} error(s), {doctor['counts']['warning']} warning(s)"
    )
    internal = conformance["internal"]["summary"]
    output(f"AQG self-conformance: {internal['passed']}/{internal['total']} cases passed")
    output(
        "Open quality/onboarding.json next; it lists the remaining product-specific tests and behavior contracts in plain language."
    )
