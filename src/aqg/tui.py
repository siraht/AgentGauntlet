...
def _draw(stdscr: Any, root: Path, message: str = "") -> None:
    ...
    try:
        ...
        review = {"summary": {"blockers": 1, "human_review": 0}, "findings": [{"severity": "blocker", "title": str(exc), "detail": ""}]}
    height, width = stdscr.getmaxyx()
    ...
    if latest:
        _safe_add(stdscr, 4, 2, f"{latest['profile']} · {human_duration(latest['duration_ms'])} · {latest['run_id']}", curses.A_DIM)
    ...
    _safe_add(stdscr, 3, 34, f"{risk_name} · {blockers} review blocker(s) · {setup_blockers} setup", curses.A_BOLD | _color("blocker" if blockers or risk_errors or setup_blockers else "pass"))
    _safe_add(stdscr, 4, 34, f"{prompts} human prompt(s) · approvals {approval_status}" + (" · onboarding stale" if onboarding_bundle["stale"] else ""), _color("review" if prompts or approval_status != "current" or onboarding_bundle["stale"] else "pass"))
    row = 6
    ...
    for index, gate in enumerate(gates):
        ...
        _safe_add(stdscr, gate_row, gate_col + 2, f"{gate['name']:<23} {human_duration(gate['duration_ms'])}")
    ...
    for finding in review.get("findings", [])[: max(2, height - row - 5)]:
        ...
        if finding.get("detail") and row < height - 4:
            for line in textwrap.wrap(finding["detail"], max(30, width - 18))[:2]:
                ...
                row += 1
    _safe_add(stdscr, height - 2, 1, "[f] fast [p] PR [d] deep [R] release [x] risk [r] review [c] conformance [s] setup [u] refresh setup [o] refresh [q] quit", curses.A_REVERSE)
    if message:
...
def _run(stdscr: Any, root: Path) -> None:
    ...
    while True:
        ...
        if key == ord("s"):
            ...
            message = f"setup next: {action.get('next_step', 'no action')}" + (" · stored plan stale" if bundle["stale"] else "")
            continue
        if key == ord("u"):
        ...
        if key == ord("x"):
            ...
            profile = {ord("f"): "fast", ord("p"): "pr", ord("d"): "deep", ord("R"): "release"}.get(key)
            if profile:
                profiles = [profile]
        if profiles:
            profile = profiles[0] if len(profiles) == 1 else "+".join(profiles)
            stdscr.erase()
            _safe_add(stdscr, 1, 2, f"Running AQG {profile}; output is captured as evidence…", curses.A_BOLD)
            stdscr.refresh()
            summaries = []
            final_status = "pass"
            for selected in profiles:
                _, summary = run_profile(root, load_policy(root), selected, keep_going=True, quiet=True)
...
def run_tui(root: Path) -> None:
    try:
        ...
        print(f"Latest: {latest['status']} · {latest['profile']}" if latest else "No evidence runs yet.")
