...
from .project import excludes, gate_applicable, load_project, source_paths, test_paths
from .review import analyze_review, review_exit_code, write_review_packet
from .util import CommandResult, git_changed_files, git_diff, human_duration, iter_files, read_json, run_command, sha256_file, utc_now, write_json
...
def _tool(root: Path, name: str, ecosystem: str) -> str:
    ...
    if not path.exists():
        raise ConfigurationError(f"missing {name} in the AQG {ecosystem} toolchain; run `python3 quality/qg.py tools install`")
    return str(path)


def _relative_files(root: Path, suffixes: set[str], project: dict[str, Any], *, tests: bool | None = None) -> list[str]:
...
def _base_ref(project: dict[str, Any]) -> str:
    return os.environ.get("AQG_DIFF_BASE") or str(project.get("enforcement", {}).get("base_ref", "HEAD"))
...
def _effective_thresholds(project: dict[str, Any]) -> dict[str, Any]:
    ...
    return _deep_merge(project.get("thresholds", {}), overrides if isinstance(overrides, dict) else {})
...
def _changed_production_files(root: Path, project: dict[str, Any], suffixes: set[str]) -> list[str]:
    return [
        ...
        if Path(path).suffix.lower() in suffixes and not _is_test_path(path) and (root / path).is_file()
    ]
...
def _run_many(
    root: Path,
    commands: Iterable[tuple[list[str], int, dict[str, str] | None]],
    *,
    stop_on_failure: bool = False,
) -> tuple[int, list[dict[str, Any]]]:
    final = PASS
    results: list[dict[str, Any]] = []
    for command, timeout, env in commands:
        result = run_command(command, cwd=root, timeout=timeout, env=env)
        results.append(result.as_dict())
        mapped = PASS if result.code == 0 else QUALITY_FAILURE if result.code == 1 else int(result.code)
        if mapped not in {PASS, QUALITY_FAILURE, CONFIGURATION_ERROR, INFRASTRUCTURE_ERROR}:
            mapped = INFRASTRUCTURE_ERROR
...
def _write_report(root: Path, gate: str, code: int, details: dict[str, Any]) -> tuple[int, dict[str, Any]]:
...
def _not_applicable(root: Path, gate: str, reason: str) -> tuple[int, dict[str, Any]]:
    return _write_report(root, gate, PASS, {"applicability": "not_applicable", "reason": reason, "commands": []})
...
def _format(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if stacks.get("javascript") or stacks.get("html") or stacks.get("css"):
        files = _scoped_files(root, project, _relative_files(root, JS_SUFFIXES | WEB_SUFFIXES | {".json", ".md", ".yaml", ".yml"}, project))
    ...
    if stacks.get("python"):
        ...
        if files:
            ...
            commands.append(([ruff, "format", "--check", "--config", "quality/config/python/ruff.toml", *files], 300, None))
    code, results = _run_many(root, commands)
    return _write_report(root, "format", code, {"applicability": "applicable", "scope": "changed" if _adopt_mode(project) else "full", "scoped_files": scoped, "commands": results})
...
def _lint(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if js_files:
        ...
        for group in _chunks(js_files):
            commands.append(([eslint, "--config", "quality/tools/js/config/eslint.config.mjs", "--report-unused-disable-directives", "--max-warnings", "0", *group], 600, None))
    css_files = _scoped_files(root, project, _relative_files(root, {".css", ".scss", ".sass", ".less"}, project))
    scoped["css"] = css_files
    if css_files:
        stylelint = _tool(root, "stylelint", "js")
        for group in _chunks(css_files):
            commands.append(([stylelint, "--config", "quality/tools/js/config/stylelint.config.mjs", *group], 600, None))
    ...
    if html_files:
        ...
        for group in _chunks(html_files):
            commands.append(([html_validate, "--config", "quality/tools/js/config/htmlvalidate.json", *group], 600, None))
    if project["stacks"].get("python"):
        ...
        if py_files:
            ruff = _tool(root, "ruff", "python")
            commands.append(([ruff, "check", "--config", "quality/config/python/ruff.toml", *py_files], 600, None))
    code, results = _run_many(root, commands)
    return _write_report(root, "lint", code, {"applicability": "applicable", "scope": "changed" if _adopt_mode(project) else "full", "scoped_files": scoped, "commands": results})
...
def _typecheck(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if project["stacks"].get("typescript"):
        ...
        config = "tsconfig.json" if (root / "tsconfig.json").exists() else "quality/config/js/tsconfig.aqg.json"
        commands.append(([tsc, "--noEmit", "--pretty", "false", "-p", config], 900, None))
    if project["stacks"].get("python"):
        mypy = _tool(root, "mypy", "python")
        py_targets = _existing_paths(root, source_paths(project))
        if not py_targets:
            raise ConfigurationError("Python typecheck is applicable but no configured source path exists")
        commands.append(([mypy, "--config-file", "quality/config/python/mypy.ini", *py_targets], 900, None))
    code, results = _run_many(root, commands)
    return _write_report(root, "typecheck", code, {"applicability": "applicable", "commands": results})
...
def _test_integrity(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if project["stacks"].get("python"):
        ...
        existing = [path for path in project.get("python", {}).get("test_paths", ["tests", "test"]) if (root / path).exists()]
        if existing:
            commands.append(([pytest, "--collect-only", "-q", "--strict-config", "--strict-markers", *existing], 600, {"PYTHONHASHSEED": "0"}))
    if project["stacks"].get("javascript") and project.get("javascript", {}).get("test_runner") == "vitest":
        vitest = _tool(root, "vitest", "js")
        commands.append(([vitest, "list", "--config", "quality/tools/js/config/vitest.config.mjs"], 600, {"CI": "1", "TZ": "UTC"}))
    command_code, results = _run_many(root, commands)
    code = max(command_code, QUALITY_FAILURE if scan["errors"] else PASS)
    return _write_report(root, "test_integrity", code, {"applicability": "applicable", "integrity": scan, "commands": results})
...
def _unit(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if project["stacks"].get("javascript"):
        ...
        if isinstance(command, list) and command:
            commands.append((_expand_project_command(root, [str(value) for value in command]), 1800, {"CI": "1", "TZ": "UTC", "FORCE_COLOR": "0"}))
        else:
            vitest = _tool(root, "vitest", "js")
            commands.append(([vitest, "run", "--config", "quality/tools/js/config/vitest.config.mjs"], 1800, {"CI": "1", "TZ": "UTC"}))
    if project["stacks"].get("python"):
        pytest = _tool(root, "pytest", "python")
        existing = [path for path in project.get("python", {}).get("test_paths", ["tests", "test"]) if (root / path).exists()]
        commands.append(([pytest, "--strict-config", "--strict-markers", "-ra", "--timeout=120", *existing], 1800, {"PYTHONHASHSEED": "0", "TZ": "UTC"}))
    code, results = _run_many(root, commands)
...
def _structure(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    commands: list[tuple[list[str], int, dict[str, str] | None]] = []
    scoped: dict[str, list[str]] = {}
    js_files = _scoped_files(root, project, _relative_files(root, JS_SUFFIXES, project, tests=False))
    scoped["javascript"] = js_files
    if js_files:
        eslint = _tool(root, "eslint", "js")
        for group in _chunks(js_files):
            commands.append(([eslint, "--config", "quality/tools/js/config/eslint.config.mjs", "--max-warnings", "0", *group], 900, None))
    radon_payload: Any = None
    radon_dict: dict[str, Any] | None = None
    if project["stacks"].get("python"):
        py_files = _scoped_files(root, project, _relative_files(root, PY_SUFFIXES, project, tests=False))
        scoped["python"] = py_files
        if py_files:
            xenon = _tool(root, "xenon", "python")
            radon = _tool(root, "radon", "python")
            commands.append(([xenon, "--max-absolute", "B", "--max-modules", "B", "--max-average", "A", *py_files], 900, None))
    ...
    if radon_dict:
        ...
        code = max(code, PASS if radon_dict["code"] == 0 else QUALITY_FAILURE if radon_dict["code"] == 1 else INFRASTRUCTURE_ERROR)
    return _write_report(root, "structure", code, {"applicability": "applicable", "scope": "changed" if _adopt_mode(project) else "full", "scoped_files": scoped, "commands": results, "radon": radon_payload})
...
def _python_coverage_metrics(root: Path, path: Path, project: dict[str, Any]) -> dict[str, Any]:
    ...
    failures: list[str] = []
    changed_production = [path for path in changed if path.endswith(".py") and not _is_test_path(path)]
    if not _adopt_mode(project):
    ...
    if changed_production and changed_pct is None:
        ...
        failures.append(f"changed-line coverage {changed_pct:.1f}% < {thresholds['changed_lines']}%")
    return {"mode": "changed" if _adopt_mode(project) else "full", "lines": line_pct, "branches": branch_pct, "changed_lines": changed_pct, "changed_executable_lines": relevant, "changed_production_files": changed_production, "failures": failures}


def _js_coverage_metrics(root: Path, summary_path: Path, final_path: Path, project: dict[str, Any]) -> dict[str, Any]:
    summary = read_json(summary_path)
    total = summary.get("total", {})
    metrics = {name: float(total.get(name, {}).get("pct", 0.0)) for name in ("lines", "branches", "functions", "statements")}
    changed = _changed_lines(root, project)
    relevant = covered = 0
    ...
    thresholds = _effective_thresholds(project)["coverage"]
    failures: list[str] = []
    changed_production = [path for path in changed if Path(path).suffix.lower() in JS_SUFFIXES and not _is_test_path(path)]
    if not _adopt_mode(project):
    ...
    if changed_production and metrics["changed_lines"] is None:
        failures.append("changed JavaScript/TypeScript production lines are absent from coverage evidence")
    elif metrics["changed_lines"] is not None and metrics["changed_lines"] < float(thresholds["changed_lines"]):
        failures.append(f"changed-line coverage {metrics['changed_lines']:.1f}% < {thresholds['changed_lines']}%")
...
def _python_crap(root: Path, project: dict[str, Any], coverage_path: Path, files: list[str] | None = None) -> dict[str, Any]:
    radon = _tool(root, "radon", "python")
    ...
    if result.code != 0:
        return {"error": result.stderr or result.stdout, "functions": [], "failures": ["radon could not produce complexity evidence"]}
    try:
        complexity = json.loads(result.stdout)
        coverage = read_json(coverage_path)
    except (json.JSONDecodeError, ConfigurationError) as exc:
        return {"error": str(exc), "functions": [], "failures": ["complexity or coverage evidence is unreadable"]}
    ...
    for filename, blocks in complexity.items():
        data = coverage_files.get(filename) or coverage_files.get(str((root / filename).resolve())) or {}
        ...
        for block in blocks:
            ...
            item = {"path": filename, "name": block.get("name"), "line": start, "end_line": end, "complexity": complexity_value, "coverage": coverage_fraction * 100, "crap": score}
            functions.append(item)
            if score > maximum:
                failures.append(f"{filename}:{start} {block.get('name')} CRAP {score:.1f} > {maximum:.1f}")
...
def _coverage(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if project["stacks"].get("javascript"):
        ...
        if isinstance(custom, list) and custom:
            ...
            command = [_tool(root, "vitest", "js"), "run", "--coverage", "--config", "quality/tools/js/config/vitest.config.mjs"]
        else:
            raise ConfigurationError("JavaScript coverage needs javascript.coverage_command or a Vitest test runner")
        result = run_command(command, cwd=root, timeout=2400, env={"CI": "1", "TZ": "UTC"})
        commands.append(result.as_dict())
        final = max(final, PASS if result.code == 0 else QUALITY_FAILURE if result.code == 1 else INFRASTRUCTURE_ERROR)
        ...
        if result.code == 0 and summary.exists():
            ...
            metrics["javascript"] = {"failures": ["coverage command passed but coverage-summary.json was not produced"]}
    if project["stacks"].get("python"):
        pytest = _tool(root, "pytest", "python")
        coverage_path = root / ".aqg" / "work" / "coverage" / "python-coverage.json"
        coverage_path.parent.mkdir(parents=True, exist_ok=True)
        existing_tests = [path for path in project.get("python", {}).get("test_paths", ["tests", "test"]) if (root / path).exists()]
        cov_args = [argument for source in project.get("python", {}).get("source_paths", source_paths(project)) for argument in ("--cov", source)]
        command = [pytest, "--strict-config", "--strict-markers", "--cov-branch", *cov_args, f"--cov-report=json:{coverage_path}", "--cov-report=term-missing", *existing_tests]
        result = run_command(command, cwd=root, timeout=2400, env={"PYTHONHASHSEED": "0", "TZ": "UTC"})
        commands.append(result.as_dict())
        final = max(final, PASS if result.code == 0 else QUALITY_FAILURE if result.code == 1 else INFRASTRUCTURE_ERROR)
        if result.code == 0 and coverage_path.exists():
            py_metrics = _python_coverage_metrics(root, coverage_path, project)
            crap_targets = _scoped_files(root, project, _relative_files(root, PY_SUFFIXES, project, tests=False))
    ...
    return _write_report(root, "coverage", final, {"applicability": "applicable", "commands": commands, "metrics": metrics})


def _contract_paths(root: Path) -> list[str]:
    return [path for path in ("tests/contracts", "test/contracts", "contracts") if (root / path).exists()]
...
def _contracts(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    commands: list[tuple[list[str], int, dict[str, str] | None]] = []
    if project["stacks"].get("python"):
        commands.append(([_tool(root, "pytest", "python"), "--strict-config", "--strict-markers", "-ra", *paths], 1800, {"PYTHONHASHSEED": "0"}))
    if project["stacks"].get("javascript"):
        commands.append(([_tool(root, "vitest", "js"), "run", "--config", "quality/tools/js/config/vitest.config.mjs", *paths], 1800, {"CI": "1"}))
    code, results = _run_many(root, commands)
    return _write_report(root, "contracts", code, {"applicability": "applicable", "commands": results})
...
def _acceptance(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if isinstance(custom, list) and custom:
        result = run_command(_expand_project_command(root, [str(value) for value in custom]), cwd=root, timeout=2400, env={"CI": "1", "TZ": "UTC"})
        commands.append(result.as_dict())
        final = max(final, PASS if result.code == 0 else QUALITY_FAILURE if result.code == 1 else INFRASTRUCTURE_ERROR)
        executed = True
    browser_files = [path for path in _relative_files(root, {".js", ".mjs", ".ts"}, project, tests=True) if any(token in path.lower() for token in ("e2e", "acceptance", "aqg-browser"))]
    if project["stacks"].get("html") or project.get("web", {}).get("start_command"):
        if browser_files:
            result = run_command([_tool(root, "playwright", "js"), "test", "--config", "quality/tools/js/config/playwright.config.mjs"], cwd=root, timeout=2400, env={"CI": "1", "TZ": "UTC"})
            commands.append(result.as_dict())
            final = max(final, PASS if result.code == 0 else QUALITY_FAILURE if result.code == 1 else INFRASTRUCTURE_ERROR)
            executed = True
    if project["stacks"].get("python"):
        python_acceptance = [path for path in _relative_files(root, {".py"}, project, tests=True) if "acceptance" in Path(path).name.lower() or "e2e" in path.lower()]
        if python_acceptance:
            result = run_command([_tool(root, "pytest", "python"), "--strict-config", "--strict-markers", "-ra", *python_acceptance], cwd=root, timeout=2400, env={"PYTHONHASHSEED": "0", "TZ": "UTC"})
            commands.append(result.as_dict())
            final = max(final, PASS if result.code == 0 else QUALITY_FAILURE if result.code == 1 else INFRASTRUCTURE_ERROR)
            executed = True
    if not executed:
        final = max(final, CONFIGURATION_ERROR)
    return _write_report(root, "acceptance", final, {"applicability": "applicable", "feature_lint": lint, "executed": executed, "commands": commands, "configuration_error": None if executed else "No executable acceptance command or test suite was found."})
...
def _copy_for_mutmut(root: Path, destination: Path) -> None:
    ...
    ignore = shutil.ignore_patterns(".git", ".aqg", ".venv", "venv", "node_modules", "quality/tools", "quality/_aqg", "dist", "build", "coverage", "htmlcov", "mutants", "__pycache__", "*.pyc")
...
def _append_mutmut_config(project_copy: Path, project: dict[str, Any], only_mutate: list[str]) -> None:
    pyproject = project_copy / "pyproject.toml"
    original = pyproject.read_text(encoding="utf-8") if pyproject.exists() else "[project]\nname='aqg-mutation-sandbox'\nversion='0.0.0'\n"
    ...
    if "[tool.mutmut]" not in original:
        ...
        original += "pytest_add_cli_args_test_selection = " + json.dumps([path for path in tests if (project_copy / path).exists()]) + "\n"
...
def _mutation_python(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    if os.name == "nt":
        return CONFIGURATION_ERROR, {"configuration_error": "mutmut requires fork support; run the mutation gate inside WSL on Windows"}
    changed = _changed_production_files(root, project, PY_SUFFIXES) if _effective_thresholds(project)["mutation"].get("changed_only", True) else _relative_files(root, PY_SUFFIXES, project, tests=False)
    if not changed:
        return PASS, {"scope": "changed", "mutated_files": [], "reason": "no changed Python production files"}
    ...
    run = run_command([mutmut, "run"], cwd=work, timeout=7200, env={"PYTHONPATH": python_path, "PYTHONHASHSEED": "0", "TZ": "UTC"})
    results = run_command([mutmut, "results"], cwd=work, timeout=300, env={"PYTHONPATH": python_path})
    text = run.stdout + "\n" + run.stderr + "\n" + results.stdout + "\n" + results.stderr
    survivors = 0
    ...
    listed = [line for line in results.stdout.splitlines() if re.search(r"(?i)(survived|x_.*__mutmut_)", line)]
    ...
    if run.code not in {0, 1} or results.code not in {0, 1}:
        ...
        code = PASS
    return code, {"scope": "changed", "mutated_files": changed, "run": run.as_dict(), "results": results.as_dict(), "survivors": survivors, "maximum_survivors": maximum, "survivor_lines": listed[:200]}
...
def _mutation_js(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    changed = _changed_production_files(root, project, JS_SUFFIXES) if _effective_thresholds(project)["mutation"].get("changed_only", True) else _relative_files(root, JS_SUFFIXES, project, tests=False)
    if not changed:
        return PASS, {"scope": "changed", "mutated_files": [], "reason": "no changed JavaScript/TypeScript production files"}
    ...
    result = run_command([stryker, "run", str(config_path)], cwd=root, timeout=7200, env={"CI": "1", "TZ": "UTC"})
    ...
    score = 100.0 if denominator == 0 and statuses else (killed * 100 / denominator if denominator else 0.0)
    ...
    if not report_path.exists() or not statuses:
        ...
        code = QUALITY_FAILURE if failures or result.code == 1 else PASS if result.code == 0 else INFRASTRUCTURE_ERROR
    return code, {"scope": "changed", "mutated_files": changed, "command": result.as_dict(), "report": payload, "status_counts": counts, "mutation_score": score, "minimum_score": threshold, "maximum_survivors": maximum, "failures": failures}
...
def _mutation_changed(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if project["stacks"].get("python"):
        ...
        final = max(final, code)
    return _write_report(root, "mutation_changed", final, {"applicability": "applicable", **details})


def _mutation_acceptance(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    code, report = run_acceptance_mutation(root)
    return _write_report(root, "mutation_acceptance", code, {"applicability": "applicable", "mutation": report})
...
def _review(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    return _write_report(root, "review", code, {"applicability": "applicable", "packet": packet, "artifacts": paths})
...
def _security_fast(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if project["stacks"].get("python"):
        ...
        commands.append(([bandit, "-q", "-r", "-c", "quality/config/python/bandit.yaml", *_existing_paths(root, source_paths(project))], 1200, None))
    code, results = _run_many(root, commands)
    ...
    if tool_lock.exists():
        result = run_command(["npm", "audit", "--audit-level=high"], cwd=tool_dir, timeout=1200, env={"CI": "1"})
        results.append(result.as_dict())
        code = max(code, PASS if result.code == 0 else QUALITY_FAILURE if result.code == 1 else INFRASTRUCTURE_ERROR)

    secret_report = scan_secrets(root, project, changed_only=True)
    code = max(code, QUALITY_FAILURE if secret_report["errors"] else PASS)
    return _write_report(root, "security_fast", code, {"applicability": "applicable", "commands": results, "changed_secret_scan": secret_report})


def _dangerous_code_scan(root: Path, project: dict[str, Any]) -> dict[str, Any]:
    patterns = [
        ("dynamic-code", re.compile(r"\b(?:eval|exec)\s*\(")),
        ("shell-execution", re.compile(r"(?i)(shell\s*=\s*true|child_process\.(?:exec|execSync)\s*\()")),
        ...
        for line_no, line in enumerate(content.splitlines(), 1):
            for code, pattern in patterns:
                if pattern.search(line) and "AQG_REVIEWED_SECURITY" not in line:
                    findings.append({"code": code, "path": rel, "line": line_no, "message": line.strip()[:240]})
...
def _security_deep(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    if project["stacks"].get("python"):
        bandit = _tool(root, "bandit", "python")
        commands.append(([bandit, "-r", "-lll", "-iii", "-c", "quality/config/python/bandit.yaml", *source_paths(project)], 1800, None))
    ...
    return _write_report(root, "security_deep", code, {"applicability": "applicable", "commands": results, "dangerous_patterns": dangerous})
...
def _start_web(root: Path, command: list[str], url: str) -> tuple[subprocess.Popen[str] | None, Path]:
    log_path = root / ".aqg" / "work" / "performance" / "web-server.log"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    if _url_ready(url):
        return None, log_path
    handle = log_path.open("w", encoding="utf-8")
    kwargs: dict[str, Any] = {"cwd": root, "stdout": handle, "stderr": subprocess.STDOUT, "text": True}
    if os.name != "nt":
...
def _performance(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    try:
        config_path = root / ".aqg" / "work" / "performance" / "lighthouserc.json"
        config = read_json(root / "quality" / "tools" / "js" / "config" / "lighthouserc.json")
        config["ci"]["collect"]["url"] = [url]
        config["ci"]["upload"]["outputDir"] = str(root / ".aqg" / "work" / "lighthouse")
        performance_thresholds = _effective_thresholds(project).get("performance", {})
        assertions = config["ci"]["assert"]["assertions"]
        assertions["categories:performance"] = ["error", {"minScore": float(performance_thresholds.get("lighthouse_performance", 0.8))}]
        assertions["categories:accessibility"] = ["error", {"minScore": float(performance_thresholds.get("lighthouse_accessibility", 0.95))}]
        write_json(config_path, config)
        lhci = _tool(root, "lhci", "js")
        result = run_command([lhci, "autorun", "--config", str(config_path)], cwd=root, timeout=3600, env={"CI": "1", "TZ": "UTC"})
        code = PASS if result.code == 0 else QUALITY_FAILURE if result.code == 1 else INFRASTRUCTURE_ERROR
    finally:
        _stop_process(process)
    return _write_report(root, "performance", code, {"applicability": "applicable", "base_url": url, "server_log": str(log), "command": result.as_dict()})


def _directory_manifest(path: Path) -> dict[str, str]:
    if not path.exists():
        return {}
    return {file.relative_to(path).as_posix(): sha256_file(file) for file in sorted(path.rglob("*")) if file.is_file()}
...
def _reproducible_build(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    output_candidates = [Path(value) for value in project.get("build_outputs", ["dist", "build", ".next", "out"])]
        ...
        for output in output_candidates:
            ...
            if target.exists():
                if target.is_dir():
                    ...
                    target.unlink()
        result = run_command(_expand_project_command(root, [str(value) for value in command]), cwd=root, timeout=3600, env={"CI": "1", "TZ": "UTC", "SOURCE_DATE_EPOCH": "0"})
        commands.append(result.as_dict())
        if result.code != 0:
            final = QUALITY_FAILURE if result.code == 1 else INFRASTRUCTURE_ERROR
            break
        manifests.append({str(output): _directory_manifest(root / output) for output in output_candidates if (root / output).exists()})
    ...
    return _write_report(root, "reproducible_build", final, {"applicability": "applicable", "commands": commands, "manifests_equal": len(manifests) == 2 and manifests[0] == manifests[1], "manifests": manifests})
...
def _release_readiness(root: Path, project: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    for required in (root / ".github" / "workflows" / "quality-gauntlet.yml", root / ".github" / "CODEOWNERS"):
    ...
    return _write_report(root, "release_readiness", code, {"applicability": "applicable", "risk": risk, "review": packet["summary"], "approvals": approvals, "findings": findings})
...
def run_adapter(root: Path, gate: str) -> tuple[int, dict[str, Any]]:
    ...
    print(f"{gate}: {report['status']}" + (" · " + ", ".join(summary_parts) if summary_parts else ""))
    if code != PASS:
