...
def _diagnose_policy(root: Path, items: list[Diagnostic]) -> dict[str, Any] | None:
    """Load and validate policy; return None when diagnose must stop early."""
...
def _diagnose_project(root: Path, items: list[Diagnostic]) -> dict[str, Any] | None:
    """Load and validate project config; return None when diagnose must stop early."""
    try:
...
def _diagnose_stack_alignment(
    root: Path, project: dict[str, Any], items: list[Diagnostic]
) -> dict[str, Any]:
    """Compare configured stacks to detection; emit drift or match diagnostic."""
...
def _diagnose_runtime_version(project: dict[str, Any], items: list[Diagnostic]) -> None:
    """Warn when project.json was generated by a different AQG version."""
...
def _version_marker_present(path: Path) -> bool:
    text = path.read_text(encoding="utf-8", errors="replace")
    return f'__version__ = "{__version__}"' in text


def _diagnose_local_runtime(root: Path, items: list[Diagnostic]) -> None:
    """Check vendored quality/_aqg or source-checkout runtime version bytes."""
    local_runtime = root / "quality" / "_aqg" / "constants.py"
    source_runtime = root / "src" / "aqg" / "constants.py"
    if local_runtime.exists():
        if _version_marker_present(local_runtime):
            _diag(
        ...
        return
    if source_runtime.exists() and _version_marker_present(source_runtime):
...
def _diagnose_project_launcher(root: Path, items: list[Diagnostic]) -> None:
...
def _diagnose_project_command(root: Path, items: list[Diagnostic]) -> None:
    command = root / "aqg"
...
def _diagnose_risk_card(
    root: Path, policy: dict[str, Any], items: list[Diagnostic]
) -> dict[str, Any]:
    """Validate change-risk card and append valid/invalid diagnostics."""
...
def diagnose(root: Path, *, strict_tools: bool = False) -> dict[str, Any]:
    ...
    policy = _diagnose_policy(root, items)
    if policy is None:
        return _report(root, items)

    project = _diagnose_project(root, items)
    if project is None:
        return _report(root, items)

    detected = _diagnose_stack_alignment(root, project, items)
    _diagnose_runtime_version(project, items)
    _diagnose_local_runtime(root, items)
    _diagnose_project_launcher(root, items)
    _diagnose_project_command(root, items)
    risk = _diagnose_risk_card(root, policy, items)
