...
export default {
  ...
  concurrency: Math.max(
    1,
    Math.min(4, Number(process.env.AQG_MUTATION_WORKERS || 2)),
  ),
  checkers:
    project.stacks?.typescript && fs.existsSync("tsconfig.json")
      ? ["typescript"]
      : [],
  tsconfigFile: fs.existsSync("tsconfig.json")
    ? "tsconfig.json"
    : "quality/config/js/tsconfig.aqg.json",
