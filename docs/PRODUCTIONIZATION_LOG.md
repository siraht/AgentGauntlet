...
| Coverage and complexity debt   | Active   | 125 tests plus 89 subtests pass; current whole-tree coverage is 58.26% / 45.44% with 42 complexity blockers; the complete branch campaign scored 54.70%; the recent hardening slice killed all 141 selected mutants with zero survivors or incomplete work; deep run `20260727-131536-ae2c20d0` passed 13 gates, retained three quality failures, and refused the 4,417-line whole-PR mutation scope with structured exit 2 and no infrastructure errors. |
| End-to-end dogfood             | Complete | Source dogfood passes cold discovery, setup, four review artifacts, PTY TUI, authenticated loopback dashboard actions, and 8/8 conformance cases. The extracted portable installer also creates working `./aqg` and Python launchers, reports zero doctor errors before tool installation, and leaves a committed fixture clean after repeated commands.                                                                                                  |
| Release and provenance         | Active   | Two candidate builds reproduce byte-for-byte; every `SHA256SUMS` entry passes. Candidate digests are `545ae6b7…85b2e` for `aqg.pyz` and `699d95e0…adb1` for the portable ZIP, with clean source provenance bound to `a117156`. Final-revision rebuild and fail-closed hosted confirmation remain.                                                                                                                                                         |
...
- Artifact dogfood must use the documented shortest path, not only an internal fallback.
  Running the extracted installer exactly as an operator would found a missing `./aqg`
  command that source and unit-level CLI checks had not exposed.
- Workspace-integrity checks expose adoption friction ordinary assertions miss. Generated
  tooling must ignore its own deterministic runtime caches before requiring an adopter's
  repository to remain clean.
...
- Reproducibility input hardening must be exercised inside mutation sandboxes. A Git-only
  ...
  repository metadata.
- Changed-function mutation needs two scopes: changed lines determine which functions are
...
- Mutation dogfood must include the clean baseline under instrumentation. A unit test that
  temporarily changed global `os.name` passed normally but broke `pathlib` inside mutmut; the
  resulting nonzero command and 139 unchecked mutants correctly produced infrastructure error.
  A pure platform predicate made the test isolated and the rerun killed all 141 mutants.
- Delegated model output is analysis, not evidence. Grok 4.5 saved primary-model tokens on
  mutmut research and adversarial test-gap review, but incomplete approval-loop sessions were
  discarded and every accepted claim was reproduced by local source inspection and tests.
